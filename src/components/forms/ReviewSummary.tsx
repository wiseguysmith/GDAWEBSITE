...
function labelFor(flow: FlowDefinition, stepIndex: number, name: string, value: unknown): string {
  const field = flow.steps[stepIndex].fields.find((f) => f.name === name);
  if (value === undefined || value === null || value === "" || (Array.isArray(value) && value.length === 0)) return "—";
  if (typeof value === "boolean") return value ? "Yes" : "No";
  if (field && (field.kind === "country" || field.kind === "conditional-country")) return countryName(String(value));
        ...
        return (
          <div key={step.id} className="grid gap-3 border-b border-rule py-5 md:grid-cols-[1fr_auto] md:gap-x-8">
            <div className="flex flex-col gap-3">
              <p className="text-eyebrow text-fg-3 tabular">
              ...
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <div key={r.name} className="grid gap-0.5 sm:grid-cols-[220px_1fr] sm:gap-x-6">
                    ...
                    <dd className="m-0 text-body text-fg">{labelFor(flow, i, r.name, r.value)}</dd>
                  </div>
                ))
              )}
            </div>
