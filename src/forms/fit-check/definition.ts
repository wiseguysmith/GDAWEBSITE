 ...
 * not in a component (handoff §36). Built per locale so every label comes
 * from that locale's content.
 */
export function fitCheckFlow(locale: Locale): FlowDefinition {
    ...
    steps: [
      {
        ...
        fields: [
          { kind: "choice", name: "projectType", options: c.steps.projectType.options, presentation: "tiles", required: true },
          {
            ...
            label: c.steps.projectType.otherLabel,
            when: { field: "projectType", equals: "other" },
            required: true,
          },
        ],
      },
      ...
      },
      {
        ...
        fields: [
          { kind: "choice", name: "relationship", options: c.steps.relationship.options, required: true },
          {
            ...
            label: c.steps.relationship.otherLabel,
            when: { field: "relationship", equals: "other" },
            required: true,
          },
        ],
      },
      {
        ...
        fields: [
          { kind: "choice", name: "objective", options: c.steps.objective.options, required: true },
          {
            kind: "conditional-text",
            name: "objectiveOther",
            label: c.steps.objective.otherLabel,
            when: { field: "objective", equals: "other" },
            required: true,
          },
