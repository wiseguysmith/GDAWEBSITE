...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return pageMetadata({ ...getContent(locale).projects.meta, path: "/projects" }, locale);
}

export default async function ProjectsPage() {
  const locale = await getLocale();
  const { projects: page, shared } = getContent(locale);
  return (
    <>
      <PageHero eyebrow={page.hero.eyebrow} heading={page.hero.heading} sub={page.hero.sub} cta={page.hero.cta} />

      <Section aria-label={page.whoFor.heading} coordinate={`§ 01 / ${page.whoFor.heading}`} ticks>
        <Container>
      ...
      <Section theme="dark" aria-labelledby="readiness" coordinate={`§ 03 / ${page.readiness.eyebrow}`} ticks>
        <Container>
      ...
      <Section aria-labelledby="prepare" coordinate={`§ 04 / ${page.prepare.eyebrow}`} ticks>
        <Container>
