...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return pageMetadata({ ...getContent(locale).investors.meta, path: "/investors" }, locale);
}

export default async function InvestorsPage() {
  const locale = await getLocale();
  const { investors: page } = getContent(locale);
  return (
    <>
      <PageHero eyebrow={page.hero.eyebrow} heading={page.hero.heading} sub={page.hero.sub} cta={page.hero.cta} />

      <Section aria-label={page.means.heading} coordinate={`§ 01 / ${page.means.heading}`} ticks>
        <Container>
      ...
      <Section theme="dark" aria-labelledby="pathway" coordinate={`§ 02 / ${page.pathway.eyebrow}`} coordinateRight={page.pathway.steps.map((s) => s.title).join(" → ")} ticks>
        <Container>
      ...
      <Section theme="white" aria-label={page.licensedRole.heading} coordinate={`§ 03 / ${page.licensedRole.heading}`} ticks>
        <Container>
